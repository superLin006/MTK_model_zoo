...
MTK DLA 适配版 Zipformer 模型定义。

修改内容 (相比原始 icefall 代码):
1. BasicNorm.forward:  ** -0.5  → torch.rsqrt()   (RSQRT 在 MDLA 5.3 支持)
...
4. EncoderMTK: 封装 Zipformer.streaming_forward，cached_len float32 输入/输出
5. DecoderNPU: 去掉 Embedding，接受 embedded float32 输入
6. JoinerMTK:  直接包装 Joiner (无修改)

使用方式:
    from zipformer_mtk_model import EncoderMTK, DecoderNPU, JoinerMTK
"""
...
_MTK_DIR   = _THIS_DIR.parent
_PROJ_ROOT = _MTK_DIR.parent
_ICEFALL_EGS = (
    _PROJ_ROOT / "icefall" / "egs" / "librispeech" / "ASR"
    / "pruned_transducer_stateless7_streaming"
)
_ICEFALL_ROOT = _PROJ_ROOT / "icefall"
...
# ===========================================================================
# 修改 2: PoolingModuleMTK — cumsum → matmul, cached_len float32
# ===========================================================================
class PoolingModuleMTK(nn.Module):
    """
    PoolingModule 的 MTK 适配版 (仅实现 streaming_forward)。
    修改:
...
# ===========================================================================
# 修改 3: RelPositionMultiheadAttentionMTK
# streaming_multi_head_attention_forward 中替换 torch.gather
# ===========================================================================
class RelPositionMultiheadAttentionMTK(nn.Module):
    """
    RelPositionMultiheadAttention 的 MTK 适配版。
    ...
    def streaming_forward(
        ...
        (
            ...
            cached_key,
            cached_val,
        ) = self.streaming_multi_head_attention_forward_mtk(
            self.in_proj(x),
    ...
    def streaming_multi_head_attention_forward_mtk(
        self,
        x_proj: Tensor,
        pos: Tensor,
        attention_dim: int,
        ...
        """
        streaming_multi_head_attention_forward 的 MTK 版本。
...
# ===========================================================================
# 修改 4: ZipformerEncoderLayerMTK — 组合修改后的子模块
# ===========================================================================
class ZipformerEncoderLayerMTK(nn.Module):
    """
    ZipformerEncoderLayer 的 MTK 适配版。
    ...
    def __init__(self, orig_layer, time1: int, kv_len: int):
        ...
        # BasicNorm MTK: Pow(-0.5) → rsqrt
        self.norm_final = BasicNormMTK(orig_layer.norm_final)

        # PoolingModule MTK: cumsum → matmul, cached_len float32
        self.pooling = PoolingModuleMTK(orig_layer.pooling, T=time1)

        # RelPositionMultiheadAttention MTK: gather → slice+stack
        self.self_attn = RelPositionMultiheadAttentionMTK(
...
# ===========================================================================
# 修改 5: ZipformerEncoderMTK — 替换 layers 并转发 streaming_forward
# ===========================================================================
class ZipformerEncoderMTK(nn.Module):
    """ZipformerEncoder 的 MTK 适配版（streaming_forward only）"""
    ...
    def __init__(self, orig_encoder, time1_per_stack: int, kv_len: int):
        ...
        self.layers = nn.ModuleList([
            ZipformerEncoderLayerMTK(layer, time1=time1_per_stack, kv_len=kv_len)
...
# ===========================================================================
# 修改 6: DownsampledZipformerEncoderMTK
# ===========================================================================
class DownsampledZipformerEncoderMTK(nn.Module):
    """DownsampledZipformerEncoder 的 MTK 适配版"""
    ...
    def __init__(self, orig_ds_encoder, time1_per_stack: int, kv_len: int):
        ...
        # 内部 encoder 也替换为 MTK 版本
        self.encoder = ZipformerEncoderMTK(
...
# ===========================================================================
# EncoderMTK: 顶层封装，接受 flat 列表 states（含 float32 cached_len）
# ===========================================================================
class EncoderMTK(nn.Module):
    """
    Zipformer Encoder MTK 封装。
    ...
    def __init__(self, orig_zipformer: Zipformer):
        ...
        for i, orig_enc in enumerate(orig_zipformer.encoders):
            ...
            if ds == 1:
                ...
                enc_mtk = ZipformerEncoderMTK(orig_enc, time1, kv_len)
            else:
                enc_mtk = DownsampledZipformerEncoderMTK(orig_enc, time1, kv_len)
            self.encoders.append(enc_mtk)
...
# ===========================================================================
# JoinerMTK: 直接包装 Joiner
# ===========================================================================
class JoinerMTK(nn.Module):
    """
    Joiner 的 MTK 包装（无修改，所有算子均支持）。
...
# ===========================================================================
# 工具函数：加载 checkpoint 并构建 MTK 模型
# ===========================================================================
def build_mtk_models(checkpoint_path: str):
    """
    从 checkpoint 加载权重，构建 EncoderMTK, DecoderNPU, JoinerMTK。

    Returns:
        encoder_mtk, decoder_npu, joiner_mtk
    """
    ...
    # 构建 MTK 版本
    encoder_mtk  = EncoderMTK(encoder).eval()
    decoder_npu  = DecoderNPU(decoder).eval()
    joiner_mtk   = JoinerMTK(joiner).eval()

    print("  MTK models built OK.")
    return encoder_mtk, decoder_npu, joiner_mtk
