"""
Test TFLite conversion and inference (using MTK TFLite tools)
"""
...
# Try to import MTK TFLite executor
try:
    from mtk_converter import TFLiteExecutor
    MTK_TFLITE_AVAILABLE = True
except ImportError:
    MTK_TFLITE_AVAILABLE = False
    print("❌ Error: mtk_converter not installed")
    print("   Please make sure you're running in the MTK-superResolution conda environment")
...
def test_tflite_load():
    """Test loading TFLite model using MTK TFLiteExecutor"""
    print("\n" + "="*70)
    print("Test 1: Load TFLite Model (MTK)")
    print("="*70)

    if not MTK_TFLITE_AVAILABLE:
        print("⊘ MTK TFLite not available - skipping")
        return True
    ...
    try:
        executor = TFLiteExecutor(model_path)
        print("✓ MTK TFLite model loaded successfully")

        # Note: MTK TFLiteExecutor doesn't expose input/output details easily
        print("  Model loaded with MTK custom operators support")

        return True

    except Exception as e:
        print(f"❌ Failed to load MTK TFLite model: {e}")
        return False
...
def test_tflite_inference():
    """Test TFLite inference using MTK TFLiteExecutor"""
    print("\n" + "="*70)
    print("Test 2: MTK TFLite Inference with Real Image (256x256)")
    print("="*70)
    ...
    try:
        ...
        # Load model using MTK TFLiteExecutor
        print("Loading MTK TFLite model...")
        executor = TFLiteExecutor(model_path)
        print("✓ Model loaded with MTK custom operators support")

        # Run inference (TFLite model uses NCHW format)
        print("Running MTK TFLite inference...")
        start_time = time.time()
        output_list = executor.run([preprocessed_input.astype(np.float32)])
        inference_time = time.time() - start_time

        # Get output (MTK TFLiteExecutor returns a list)
        core_output = output_list[0] if isinstance(output_list, list) else output_list

        # Apply inverse MeanShift
        print("应用 MeanShift 后处理...")
        final_output = core_output + rgb_range * rgb_mean

        print("✓ MTK TFLite inference successful")
        print(f"  Input shape (NCHW):  {test_input.shape}")
        print(f"  Output shape (NCHW): {final_output.shape}")
        print(f"  推理时间: {inference_time:.4f}s ({inference_time*1000:.1f}ms)")

        # Save output if image was loaded
        if PIL_AVAILABLE and os.path.exists(image_path):
            output_path_img = os.path.join(output_dir, "edsr_tflite_mtk_256x256.png")
        ...
        if final_output.shape == expected_shape:
            ...
            print(f"❌ Output shape mismatch. Expected {expected_shape}, got {final_output.shape}")
            return False

    except Exception as e:
        print(f"❌ MTK TFLite inference failed: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def main():
    """Run all tests"""
    print("\n" + "="*70)
    print("EDSR MTK TFLite Tests")
    print("="*70)

    if not MTK_TFLITE_AVAILABLE:
        print("⚠ MTK TFLite not available")
        print("Please install mtk_converter and run in MTK-superResolution conda environment")
        return 1

    results = []
    results.append(("Load MTK TFLite Model", test_tflite_load()))
    results.append(("MTK TFLite Inference", test_tflite_inference()))
