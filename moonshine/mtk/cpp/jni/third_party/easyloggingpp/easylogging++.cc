...
void DateTime::gettimeofday(struct timeval* tv) {
    ...
    if (tv != nullptr) {
        ...
        const unsigned long usecOffSet = 1000000; // NOLINT [runtime/int] [4]
        FILETIME fileTime;
        ...
        present -= delta_;
        tv->tv_sec = static_cast<long>(present * secOffSet); // NOLINT [runtime/int] [4]
        tv->tv_usec = static_cast<long>(present % usecOffSet); // NOLINT [runtime/int] [4]
    }
#else
...
base::type::string_t DateTime::formatTime(unsigned long long time, // NOLINT [runtime/int] [4]
...
unsigned long long DateTime::getTimeDifference(const struct timeval& endTime, // NOLINT [runtime/int] [4]
                                               const struct timeval& startTime,
                                               base::TimestampUnit timestampUnit) {
    if (timestampUnit == base::TimestampUnit::Microsecond) {
        return static_cast<unsigned long long>( // NOLINT [runtime/int] [4]
            static_cast<unsigned long long>(1000000 * endTime.tv_sec + endTime.tv_usec) - // NOLINT [runtime/int] [4]
            static_cast<unsigned long long>(1000000 * startTime.tv_sec + // NOLINT [runtime/int] [4]
                                            startTime.tv_usec));  // NOLINT [runtime/int] [4]
    }
    // milliseconds
    auto conv = [](const struct timeval& tim) {
        return static_cast<unsigned long long>((tim.tv_sec * 1000) + // NOLINT [runtime/int] [4]
                                               (tim.tv_usec / 1000));
    };
    return static_cast<unsigned long long>(conv(endTime) - // NOLINT [runtime/int] [4]
                                           conv(startTime));
}
    ...
    for (; *format; ++format) {
        if (*format == base::consts::kFormatSpecifierChar) {
            switch (*++format) {
                ...
                case 'h':  // hour (12-hour)
                    buf =
                        base::utils::Str::convertAndAddToBuff(tInfo->tm_hour % 12, 2, buf, bufLim);
                    continue;
                case 'H':  // hour (24-hour)
...
unsigned long TypedConfigurations::getULong(std::string confVal) {  // NOLINT [runtime/int] [4]
    bool valid = true;
...
void DefaultLogDispatchCallback::dispatch(base::type::string_t&& logLine) {
    ...
    switch (m_data->logMessage()->level()) {
        case Level::Fatal:
    ...
    __android_log_print(androidLogPriority,  consts::kDefaultLoggerId, "%s", logLine.c_str());
#endif
...
#if defined(ELPP_SYSLOG)
    else if (m_data->dispatchAction() == base::DispatchAction::SysLog) { // NOLINT
...
base::type::string_t DefaultLogBuilder::build(const LogMessage* logMessage,
    ...
    if (logMessage->level() == Level::Verbose &&
        ...
        buf = base::utils::Str::convertAndAddToBuff(logMessage->verboseLevel(), 1, buf, bufLim,
                                                    false);
        base::utils::Str::replaceFirstWithEscape(
...
PerformanceTracker::PerformanceTracker(const std::string& blockName,
    ...
    // or before calling checkpoint, we still depend on state of configuration at time of
    // construction
...
void Helpers::crashAbort(int sig, const char* sourceFile,
                         unsigned int long line) {  // NOLINT [runtime/int] [4]
    std::stringstream ss;
