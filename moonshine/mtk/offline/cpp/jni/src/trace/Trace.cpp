...
ATracerAndroid::ATracerAndroid() {
    auto loader = SharedLib::Load("libandroid.so");

    if (UNLIKELY(loader == nullptr)) {
        LOG(ERROR) << "Load libandroid failed.";
        return;
    }
    mSharedLib = std::move(loader);

    mFpAtraceIsEnabled = mSharedLib->LoadFunc<FpIsEnabled>("ATrace_isEnabled");
    mFpAtraceBeginSection = mSharedLib->LoadFunc<FpBeginSection>("ATrace_beginSection");
    mFpAtraceEndSection = mSharedLib->LoadFunc<FpEndSection>("ATrace_endSection");

    if (!mFpAtraceIsEnabled || !mFpAtraceBeginSection || !mFpAtraceEndSection) {
        LOG(ERROR) << "Load function failed.";
        return;
    }
    mIsEnabled = mFpAtraceIsEnabled();
}
}  // namespace mtk::neuropilot